...
export const ActionsMenu: React.FC<ActionsMenuProps> = ({ expand, image, withoutStart, isActive }) => {
  ...
  const imagePull = useStoreActions((actions) => actions.imagePull);
  const imagePush = useStoreActions((actions) => actions.imagePush);
  const imageRemove = useStoreActions((actions) => actions.imageRemove);
