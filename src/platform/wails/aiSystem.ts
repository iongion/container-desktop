// Wails (webview-realm) caps-assembler for the AI subsystem — the thin shell adapter over the neutral
// AI composition root, mirroring platform/electron/aiSystem.ts. It ASSEMBLES the privileged host
// capabilities from the Wails platform impls (OS-keychain / env-isolating exec / DNS over `invoke`) and provides
// the window.FS/Path ports + the in-webview transport; all broker wiring lives once in the shared AI host. The IPC
// transport is injected (aiSystemHost provides an in-webview bus), so this module never imports @wailsio/runtime.
...
export interface AISystemDeps {
  /** Wails `invoke` — reaches the keychain_*, command_execute, dns_lookup host commands. */
  invoke: WailsInvoke;
...
// Assemble the Wails host capabilities + deps and hand off to the shared composition root. Async because the
...
export async function createAISystem(deps: AISystemDeps): Promise<AIBroker> {
  return createSharedAISystem(
    {
      keychain: await createWailsKeychain(deps.invoke),
