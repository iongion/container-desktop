...
// Local dialog-options shape (the Tauri path imported this from @tauri-apps/plugin-dialog). bridge.ts's
// showMessage adapter maps these fields onto @wailsio/runtime Dialogs (kind → Info/Warning/Error/Question,
// buttons → the Wails Button list whose clicked Label is returned).
...
export interface WailsRecoveryDeps {
...
export interface WailsRecoveryService {
...
export function createRecoveryService(deps: WailsRecoveryDeps): WailsRecoveryService {
  ...
  const showRecoveryDialog: WailsRecoveryService["showRecoveryDialog"] = async (title, error, options) => {
