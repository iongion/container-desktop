// In-webview AI hub for the Wails realm — the AI analog of resourceSyncHost.ts. Under Wails the AIBroker runs in
...
// window.AIBus surface (via the shared aiClientBridge) the renderer already reads. Wails-agnostic (no
// @wailsio/runtime imports) so it stays unit-testable; bridge.ts ties it to the real invoke + engineOps.
...
import { createWailsAIBus } from "./aiBus";
import { createWailsAIClient } from "./aiClient";
...
export async function createAISystemHost(deps: AISystemHostDeps): Promise<AISystemHost> {
  ...
  // The SAME typed window.AI / window.AIBus surface the renderer reads; the named Wails client/bus modules
  // mirror platform/electron/aiClient.ts and platform/electron/aiBus.ts.
  const ai = createWailsAIClient(bus);
  const aiBus = createWailsAIBus(bus);
