// The Wails-side process primitives — ExecuteStreaming / ExecuteAsBackgroundService / Spawn / Kill — backed
// by the Go process commands (src-wails/process.go). Two distinct emitter shapes (do NOT conflate):
...
//     is a JS ProxyRequest ping — it cannot move to Go), Go only spawns/streams/kills.
// Kill routes to Go by a processId token stamped onto the wrap_process child, so BOTH Command.Kill(child)
// and a direct child.kill() (services.ts) terminate the Go-owned process. Mirrors commander.ts exactly,
...
export async function exec_launcher_async(
  ...
  // Secret-bearing stdin (registry `login --password-stdin`, `cat > ca.crt`) — piped to the child by the Go
  // side (exec_service.go), never placed in argv or logged.
