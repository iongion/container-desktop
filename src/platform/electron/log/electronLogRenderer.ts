// Electron RENDERER logging adapter (one LoggerBackend behind the @/platform/logger port). It forwards already
...
// with the @/platform/logger façade.
//
...
function setup(): void {
  ...
  initialized = true;
  // Console is owned by the @/platform/logger façade; electron-log/renderer only FORWARDS to main (its ipc
...
// The persistence sink the @/platform/logger façade calls in the RENDERER: hand the record to electron-log, which
