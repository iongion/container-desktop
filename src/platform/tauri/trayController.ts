// The Tauri TrayController, JS side. The native widget is Rust (tray.rs), but the MENU is projected here from the
// SHARED buildTrayMenuTemplate (the same projection Electron's TrayController uses) fed by the in-realm
// EngineDataService.getTrayMenuData(). Electron menu templates carry click CLOSURES, which can't cross to Rust —
// so we flatten the template into (a) a serializable node tree Rust builds the native menu from, and (b) a
// registry mapping generated ids → the closures, kept in THIS realm. A native menu click emits the id back over
// a Tauri event, and we invoke the matching closure → EngineDataService.performAction. Rebuilt on every engine
...
// Serializable menu node (matches src-tauri/src/tray.rs MenuNode). A separator; a submenu (items); a clickable
...
// dark/light variant is chosen natively (Rust reads the OS theme in tray.rs).
...
// The engine-service surface the tray reads (a subset of EngineDataService; the hosted instance under Tauri).
