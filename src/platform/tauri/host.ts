import type { IFileSystem, IPath, IPlatform } from "@/platform/contract";

export type TauriInvoke = (command: string, args?: Record<string, unknown>) => Promise<any>;

export function createPlatform(invoke: TauriInvoke, osType: string): IPlatform {
  return {
    ...
    // ~/.ssh/config parsed natively (Rust) — the `ssh-config` npm parser pulls node builtins and can't bundle
    // into the webview, so it lives in the host I/O layer. Returns the SSHHost[] the scope picker lists.
    getSSHConfig: () => invoke("get_ssh_config"),
    // Launch a per-OS terminal running `launcher args...` via the Rust launch_terminal command. Normalize the
...
// Pure-JS path ops (no Rust round-trip). Cleans separators, joins, and collapses `.`/`..` segments with the
...
export function createFileSystem(invoke: TauriInvoke): IFileSystem {
  return {
    ...
    // 0600 owner-only write for AI credentials/permissions/knowledge — the native Rust command hardens the mode
    // (fs_write_private_text_file in host.rs), matching the Node impl. Best-effort on Windows (no unix mode).
