// The Tauri-side SSH control plane — Command.StartSSHConnection + the ISSHClient it returns. Entirely JS: it
// reuses the shared, pure buildSSHArgs (@/container-client/ssh-args) over the already-Rust-backed
// Command.Execute / Command.ExecuteStreaming, so NO new native code is needed for exec-over-SSH. The DATA
// plane (the engine-API dial over an SSH dial-stdio bridge / `ssh -NL` tunnel) is owned by the Rust proxy
// (src-tauri/src/proxy.rs) — hence startTunnel/startStdioBridge are inert here (Command.ProxyRequest is the
// Rust proxy-request bridge, which never calls back into this client).
//
// Why the argv needs no credential/home resolution in Rust: every SSH scope comes from ~/.ssh/config
...
export async function startSSHConnection(deps: SSHClientDeps, host: SSHHostLike, _opts?: any): Promise<ISSHClient> {
  ...
  const client: ISSHClient = {
    ...
    // Data plane is Rust-owned in the Tauri shell (proxy.rs bridges the dial-stdio / -NL tunnel), so the JS
    // client never manages tunnels. Inert — never reached, since Command.ProxyRequest is the Rust proxy bridge.
    startTunnel: async () => {
      throw new Error("SSH tunnels are managed by the Rust proxy in the Tauri shell");
    },
