...
// The Rust-side process event (src-tauri/src/process.rs ProcessEvent).
...
// Rust ProcessEvent -> the emitter events node/exec/commander.ts emits (payload shapes per contract.ts).
...
// The Tauri child carries the Rust processId so Command.Kill(child) and child.kill() route to process_kill.
