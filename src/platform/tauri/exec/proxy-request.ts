// The Tauri-side Command.ProxyRequest — the drop-in replacement for platform/electron/commandProxyClient's
// forwardProxyRequest. It re-synthesizes the EXACT shapes the unchanged createApplicationApiDriver
// (container-client/Api.clients.ts) consumes, backed by the Rust proxy commands (src-tauri/src/proxy.rs)
...
//
// Tauri's invoke + Channel are INJECTED (ProxyRequestDeps) so this whole flow is unit-testable with fakes;
// bridge.ts supplies the real @tauri-apps/api bindings. The stream emitter reuses the shared
...
// A Tauri Channel, reduced to what we use: an assignable onmessage sink passed as an invoke arg.
// The Tauri Channel also delivers raw ArrayBuffer frames (binary log data via InvokeResponseBody::Raw); the
...
export interface ProxyRequestDeps {
  ...
  /** Create a fresh Tauri Channel (passed to proxy_request_stream; its onmessage receives stream events). */
  newChannel: () => ProxyChannel;
  /** Local OS type (for the ssh/ssh.exe launcher in an SSH bridge spec). Defaults to non-Windows. */
  osType?: string;
}

// A remote-connection bridge the Rust proxy must bring up (and dial the LOCAL end of) before proxying: an SSH
// dial-stdio bridge / `ssh -NL` tunnel, or a WSL dial-stdio bridge. Built HERE from the shared arg builders
// (the source of truth for the argv) so Rust stays engine-agnostic — it only spawns `launcher argv` and shuttles
...
// resolution reaches Rust; dialStdioCommand present ⇒ a per-connection `ssh <alias> -- <cmd>` stdio bridge
...
// Translate one Rust stream event into an emit on the shared EmitterStream (mirror createForwardedStream):
...
export function applyStreamEvent(
  ...
  // Binary data chunk (container logs): Rust sent raw bytes (InvokeResponseBody::Raw). Hand them straight to the
...
/** Build the Command.ProxyRequest function from injected Tauri bindings. */
export function createProxyRequest(deps: ProxyRequestDeps) {
  return async function proxyRequest(request: any, connection: any, _context?: any): Promise<any> {
    ...
    // For an SSH/WSL remote, Rust must bring up (and dial the local end of) a bridge; direct dials are undefined.
