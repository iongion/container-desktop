...
// Engines
export const ENGINE_PODMAN_NATIVE = `${PROGRAM}.native`;
export const ENGINE_PODMAN_VIRTUALIZED = `${PROGRAM}.virtualized`;
export const ENGINE_PODMAN_SUBSYSTEM_WSL = `${PROGRAM}.subsystem.wsl`;
export const ENGINE_PODMAN_SUBSYSTEM_LIMA = `${PROGRAM}.subsystem.lima`;

export class PodmanClientEngineNative extends AbstractClientEngine {
...
export class PodmanClientEngineVirtualized extends AbstractControlledClientEngine {
  ...
  async stopApi(customSettings: any, opts?: any) {
    if (!this.apiStarted) {
...
export class PodmanClientEngineSubsystemWSL extends AbstractClientEngineSubsystemWSL {
  ...
  async getMachines(customFormat) {
    ...
    let items = [];
    if (canListScopes) {
      const command = await this.getScopedCommand(settings.program, []);
...
export class PodmanClientEngineSubsystemLIMA extends AbstractClientEngineSubsystemLIMA {
