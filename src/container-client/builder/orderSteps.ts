// Present build steps in Dockerfile order.
//
// Engines don't emit steps in Dockerfile order: `docker buildx build --progress=rawjson` emits vertices in
...
// solve order, not execution order). The reliable signal is the Dockerfile step number buildx prints in each
...
// A monotonic sort bucket: numbered layers by their Dockerfile position, internal setup before them, the
