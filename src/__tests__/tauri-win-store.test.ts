...
describe("Tauri Windows Store packaging", () => {
  ...
  it("honors PUBLISHER / PUBLISHER_DISPLAY_NAME from the environment (Microsoft Store publisher identity)", () => {
    const previousPublisher = process.env.PUBLISHER;
    const previousDisplay = process.env.PUBLISHER_DISPLAY_NAME;
    process.env.PUBLISHER = "CN=52408AA8-2ECC-4E48-9A2C-6C1F69841C79";
    process.env.PUBLISHER_DISPLAY_NAME = "Ionut Stoica";
    try {
      const identity = store.resolveStoreIdentity({ pkg, arch: "x64" });
      expect(identity.publisher).toBe("CN=52408AA8-2ECC-4E48-9A2C-6C1F69841C79");
      expect(identity.publisherDisplayName).toBe("Ionut Stoica");
      // The package family name suffix is a hash of the publisher, so the correct CN fixes it too.
      expect(store.createStoreManifest({ pkg, arch: "x64" })).toContain(
      ...
      if (previousPublisher === undefined) {
        ...
        process.env.PUBLISHER = previousPublisher;
      }
      if (previousDisplay === undefined) {
        delete process.env.PUBLISHER_DISPLAY_NAME;
      } else {
        process.env.PUBLISHER_DISPLAY_NAME = previousDisplay;
      }
    }
  });
