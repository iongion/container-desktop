...
describe("normalizeAISettings", () => {
  ...
  it("drops obsolete top-level AI settings while preserving the supported schema", () => {
    const ai = normalizeAISettings({ webSearch: true, obsoletePermissionState: { provider: "openai" } } as any);
    expect(ai.webSearch).toBe(true);
    expect(ai).not.toHaveProperty("obsoletePermissionState");
  });
