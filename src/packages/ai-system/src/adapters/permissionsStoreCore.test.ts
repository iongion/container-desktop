...
import { commandKey } from "@/ai-system/core/permissions";
...
describe("permissionsStoreCore", () => {
  ...
  it("serializes concurrent mutations without losing either decision", async () => {
    const store = createPermissionsStore(FILE, memFs().fs, path);

    await Promise.all([
      store.addCommand("allowed", { program: "podman", args: ["stop", "one"] }),
      store.addCommand("blocked", { program: "docker", args: ["remove", "two"] }),
    ]);

    const snap = await store.load();
    expect(snap.allowed).toEqual([expect.objectContaining({ program: "podman", args: ["stop", "one"] })]);
    expect(snap.blocked).toEqual([expect.objectContaining({ program: "docker", args: ["remove", "two"] })]);
  });

  it("writes through a private temporary file and keeps the last committed state after rename failure", async () => {
    const memory = memFs();
    const store = createPermissionsStore(FILE, memory.fs, path);
    await store.addCommand("allowed", { program: "podman", args: ["stop", "one"] });
    const committed = memory.files.get(FILE);
    const originalRename = memory.fs.rename;
    memory.fs.rename = vi.fn(async () => {
      throw new Error("rename failed");
    });

    await expect(store.addCommand("blocked", { program: "docker", args: ["remove", "two"] })).rejects.toThrow(
      "rename failed",
    );

    expect(memory.files.get(FILE)).toBe(committed);
    expect((await store.load()).blocked).toEqual([]);
    memory.fs.rename = originalRename;
  });
});
