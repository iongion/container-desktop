...
export interface AISystemHost<TEvent = unknown> {
  broker: AIBroker<TEvent>;
...
export async function createAISystemHost<TEvent = unknown>(deps: AISystemDeps<TEvent>): Promise<AISystemHost<TEvent>> {
