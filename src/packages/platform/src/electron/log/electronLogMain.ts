...
// Electron MAIN logging adapter (one LoggerBackend behind the @/logger port). It owns the single LOCAL
// log file + size-based rotation via electron-log, and — through log.initialize() — receives the records
// the renderer forwards. Console output stays with the @/logger façade; this adapter handles FILE only.
//
...
export function setupElectronLogMain(): void {
  ...
  initialized = true;
  log.initialize();
  // LOCAL-ONLY: never a remote/cloud sink. Console stays with the @/logger façade, and main does not echo
...
// The persistence sink the @/logger façade calls in MAIN. Console stays with the façade; this only routes
