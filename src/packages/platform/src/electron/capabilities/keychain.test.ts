...
import { createNodeKeychain, type KeychainFsLike, type SafeStorageLike } from "./keychain";
...
function memFs(): KeychainFsLike & { dump: () => Record<string, string> } {
...
describe("createNodeKeychain", () => {
  it("round-trips a key: set → has → get", async () => {
    const ks = createNodeKeychain({ safeStorage: fakeSafeStorage(), fs: memFs(), platform: "linux" });
  ...
  it("persists ciphertext, never plaintext", async () => {
    const fs = memFs();
    const ks = createNodeKeychain({ safeStorage: fakeSafeStorage(), fs, platform: "linux" });
  ...
  it("serializes concurrent key mutations without losing either provider", async () => {
    const fs = memFs();
    const ks = createNodeKeychain({ safeStorage: fakeSafeStorage(), fs, platform: "linux" });

    await Promise.all([ks.setKey("openai", "sk-one"), ks.setKey("anthropic", "sk-two")]);

    expect(await ks.getKey("openai")).toBe("sk-one");
    expect(await ks.getKey("anthropic")).toBe("sk-two");
  });

  it("reports a healthy encryption backend", () => {
    const ks = createNodeKeychain({
