...
export interface TauriCommandDeps {
...
export function createRustCommand(deps: TauriCommandDeps): ICommand {
  const notImplemented = async (): Promise<never> => {
    throw new Error("CreateNodeJSApiDriver is not implemented in the Tauri shell");
  };
...
export function createCommand(deps: TauriCommandDeps): ICommand {
  const base = isMockMode() ? createMockCommand() : createRustCommand(deps);
