// Tauri Keychain (IKeychain) — the webview-realm OS-secret-store capability, reached through the GENERIC Rust
// `keychain_*` commands (src-tauri/src/keychain.rs). The IKeychain consumer here is the AI provider-key store, so
...
export async function createTauriKeychain(invoke: TauriInvoke): Promise<IKeychain> {
  ...
  return {
    ...
    async getKey() {
      // The provider key is resolved in Rust by the native transport and is never handed to this realm. The
      // backing command is not registered, so there is nothing here to call.
      throw new Error("AI: reading the provider key is not available in the web view");
    },
    ...
    async setKey(provider, plaintext, opts) {
      ...
      // When opted in and the OS vault is unreachable, the Rust side persists to a 0600 file fallback (Electron
