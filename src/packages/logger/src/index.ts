// The minimal logger port — the logger leaf owns it; consumers import ILogger from @/logger. Imports nothing
// internal, so the logger leaf stays dependency-free (level persistence is injected by the caller, see below).
...
export function getEnvironmentLogLevel(): string | undefined {
...
// Set the active level in-memory and re-level every live logger. PURE (no persistence). A host that
// persists the level (container-client/logging) reads config, then calls this.
export function applyLevel(level?: string): LogLevel {
