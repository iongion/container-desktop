...
export const ImagesCard: React.FC<ToolCardProps> = ({ result }) => {
  const { t } = useTranslation();
  const items = (Array.isArray(result) ? result : []) as ContainerImage[];
  if (items.length === 0) {
    return <div className="AICardEmpty">{t("No images.")}</div>;
  }
  return (
