// Pure guard for windowed-list scroll restoration (see useWindowedRows). That hook persists a list's
// measured row heights + scroll offset per route key and re-seeds them on the next mount, so list -> detail
// -> back lands exactly where you left off. But the SAME route can host a DIFFERENT data set on the next
// mount: a connection can be disconnected while the screen is unmounted (the merged list is per-connection),
// so the list shrinks. Restoring the old — deeper — offset onto the now-shorter content would scroll past
// every row and window them all out of view, i.e. a large empty gap at the top and no rows. This module
// decides, from the incoming row set alone (no DOM, no virtualizer), whether a saved offset is still safe.
