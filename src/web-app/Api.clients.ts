...
export class ContainerClient {
  ...
  async getContainerLogs(id: string) {
    return this.withResult<Uint8Array>(async () => {
      const result = await this.dataApiDriver.get<Uint8Array>(`/containers/${id}/logs`, {
        params: {
          stdout: true,
          stderr: true
        },
  ...
  async getNetwork(name: string) {
    return this.withResult<Network>(async () => {
      if (this.connector?.adapter === ContainerAdapter.DOCKER) {
        const result = await this.dataApiDriver.get<Network[]>(`/networks/${encodeURIComponent(name)}`, {
  ...
  async createNetwork(opts: CreateNetworkOptions) {
    return this.withResult<Network>(async () => {
      ...
      if (this.connector?.adapter === ContainerAdapter.DOCKER) {
        ...
        // TODO: Subnets
        const result = await this.dataApiDriver.post<Network>("/networks/create", creatorDocker, {
          baseURL: "http://localhost"
        });
        if (result.ok) {
  ...
  async removeNetwork(name: string) {
    return this.withResult<boolean>(async () => {
      if (this.connector?.adapter === ContainerAdapter.DOCKER) {
        const result = await this.dataApiDriver.delete<Network[]>(`/networks/${encodeURIComponent(name)}`, {
          baseURL: "http://localhost"
        });
        return result.ok;
      }
      const result = await this.dataApiDriver.delete<boolean>(`/networks/${encodeURIComponent(name)}`, {
        baseURL: "http://d/v4.0.0/libpod"
      });
      return result.ok;
    });
