...
// project
import { RadioLabel } from "@/web-app/components/RadioLabel";
import { RestrictedTo } from "@/web-app/components/RestrictedTo";
import { useStoreActions, useStoreState } from "@/web-app/domain/types";
import { Native } from "@/web-app/Native";
...
import {
  ...
  ProgramTestResult,
  TestResult
} from "@/web-app/Types.container-app";

import { useTranslate } from "@/web-app/App.i18n";
...
export const ContainerEngineSettingsProgramLocal: React.FC<ContainerEngineSettingsProps> = ({
  connector,
  disabled
}: ContainerEngineSettingsProps) => {
  ...
  const updateConnectorByResult = useCallback(
    async (success: boolean, path?: string, version?: string, scopes?: ControllerScope[]) => {
  ...
  const onFindControllerProgram = useCallback(
    async (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
      ...
      if (result && result.path) {
        Notification.show({
          message: t("Found {{program}} CLI in {{scope}} on {{path}} {{version}}", {
            program: program.name,
            path: result.path,
        ...
        Notification.show({
          message: t("Unable to find {{program}} CLI in {{scope}}", { program: program.name, scope: values.scope }),
  ...
  const onProgramPathTestClick = useCallback(
    async (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
      const values = getValues();
  ...
  const onConnectionStringTestClick = useCallback(
    async (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
      const values = getValues();
    ...
    scopeSelectorWidget = (
      <Controller
        ...
        render={({ field: { onChange, onBlur, value, name, ref }, fieldState: { isDirty, error } }) => {
          ...
          return (
            <FormGroup
              className="ProgramScopeLocator"
              label={scopeLabel}
              labelFor="scopeSelector"
              intent={Intent.DANGER}
              helperText={scopeSelectorHelperText}
            >
              <ControlGroup>
                <HTMLSelect
                  name={name}
        ...
        render={({ field: { onChange, onBlur, value, name, ref }, fieldState: { isDirty, error } }) => {
          ...
          let helperText = availability.controller ? (
            <div className="AppSettingsFieldProgramHelper">
              {controller?.version ? (
                <span>{t("Detected version {{version}}", controller as any)}</span>
              ) : (
                  ...
                  <InputGroup
                    ...
                    rightElement={
                      <Button
                        ...
                        intent={Intent.PRIMARY}
                        text={t("Test")}
                        onClick={onProgramPathTestClick}
                      />
                    }
                  />
                  {programSelectButton}
    ...
    <div className="ContainerEngineSettings" data-settings="program.local">
      ...
      <Controller
        ...
        render={({ field: { onChange, onBlur, value, name, ref }, fieldState: { isDirty, error, invalid } }) => {
          ...
          return (
            <FormGroup label={t("Connection string")} labelFor="connectionString" helperText={message}>
              <ControlGroup fill={true} vertical={false}>
                <InputGroup
                  disabled={pending}
                  fill
                  ...
                  rightElement={
                    <ButtonGroup>
                      <Button
                        ...
                        intent={Intent.PRIMARY}
                        text={t("Test")}
                        onClick={onConnectionStringTestClick}
                      />
                    </ButtonGroup>
                  }
                />
...
export const ContainerEngineManagerSettings: React.FC<ContainerEngineManagerSettingsProps> = ({
  adapter,
  disabled,
  connectors,
  currentConnector,
  ...
  const onSaveClick = handleSubmit(async (data) => {
    ...
    const engineUserSettings: EngineConnectorSettings = {
      program: merge({}, connector.settings.current.program, {
        path: data.programPath
      }),
      api: merge({}, connector.settings.current.api, {
        connectionString: data.connectionString
      })
    };
    if (connector.settings.current.controller) {
      engineUserSettings.controller = merge({}, connector.settings.current.controller, {
        ...
        <div className="AppSettingsFormView" data-form-view="container-engine" data-adapter={adapter}>
                      ...
                      <Radio
                        ...
                        labelElement={
                          <RadioLabel
                            text={label}
                            important={important}
                            highlight={currentConnector.id === engineConnector?.id}
                          />
                        }
          ...
          <div className="AppSettingsFormViewFooter">
            ...
            <FormGroup className="ContainerEngineSettingsSetDefault">
              <ControlGroup>
                <Checkbox label={t("Use as default")} onChange={onUseAsDefaultChange} checked={useAsDefault} />
              </ControlGroup>
            </FormGroup>
            ...
            <Button
              ...
              minimal
              intent={Intent.NONE}
              title={t("Reset to automatic values")}
...
export const ContainerEngineManager: React.FC<ContainerEngineManagerProps> = ({
  ...
  const podmanConnectors = useMemo(
    () => connectors.filter((it) => it.adapter === ContainerAdapter.PODMAN),
    [connectors]
  );
  const dockerConnectors = useMemo(
    () => connectors.filter((it) => it.adapter === ContainerAdapter.DOCKER),
    [connectors]
  );
  ...
  return (
    <div className="AppSettingsEngineManager">
      <Tabs selectedTabId={containerAdapter} onChange={onContainerAdapterChange} renderActiveTabPanelOnly>
        <Tab
          id={ContainerAdapter.PODMAN}
          title={t("Podman")}
          panelClassName="podman-panel"
          panel={
            <ContainerEngineManagerSettings
              adapter={ContainerAdapter.PODMAN}
              currentConnector={currentConnector}
              engines={PodmanContainerEngines}
              connectors={podmanConnectors}
            />
          }
        />
        <Tab
          id={ContainerAdapter.DOCKER}
          title={t("Docker")}
          panelClassName="docker-panel"
          panel={
            <ContainerEngineManagerSettings
              adapter={ContainerAdapter.DOCKER}
              currentConnector={currentConnector}
              engines={DockerContainerEngines}
              connectors={dockerConnectors}
            />
          }
        />
        <Tabs.Expander />
