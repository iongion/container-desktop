...
// src file OUTSIDE the three backends src/platform/{electron,tauri,wails} — reaches the host only through
// contract-typed ports/globals, never importing node:*/electron/@tauri-apps/@wailsio/a node-coupled dep, and never using
...
// A file is a BACKEND (may use node/electron/@tauri/@wailsio) when it lives under one of these.
const BACKEND_DIRS = ["src/platform/electron/", "src/platform/tauri/", "src/platform/wails/"];
...
const FORBIDDEN_IMPORT =
  /^(node:|electron$|electron\/|@tauri-apps\/|@wailsio\/|undici$|ssh-config$|fix-path$|electron-log)/;
