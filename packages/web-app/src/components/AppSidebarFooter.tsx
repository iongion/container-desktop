...
import { Alignment, Navbar, NavbarGroup, NavbarHeading, Spinner, SpinnerSize, Intent, Button, ButtonGroup, PopoverInteractionKind, Popover } from "@blueprintjs/core";
  ...
  if (!expandSidebar) {
    rightContent = (
      <Popover
        ...
        popoverClassName="bp4-popover-content-sizing AppSidebarInfoPopover"
        position="top-left"
        content={(
          <div className="AppSidebarFooterVersionsMenu">
            <span className="AppSidebarVersionString"><strong>GUI</strong> {versionString}</span>
            <br/>
            <span className="AppSidebarProgramString" title={programTitle}><strong>CLI</strong> {programString}</span>
          </div>
        )}
        ><Button className="AppSidebarInfoButton" minimal icon={IconNames.INFO_SIGN} /></Popover>
    );
  }
  return (
    <div className="AppSidebarFooter">
      <div className="AppSidebarFooterOverlay">
        <ButtonGroup minimal>
          <Button icon={theme === AppTheme.DARK ? IconNames.MOON : IconNames.FLASH} onClick={onThemeToggleClick} title={t("Toggle {{mode}} mode", { mode: theme === AppTheme.DARK ? t("light") : t("dark") })} />
          <Button icon={expandSidebar ? IconNames.DOUBLE_CHEVRON_LEFT : IconNames.DOUBLE_CHEVRON_RIGHT} onClick={onExpandCollapseSidebarClick} title={t("{{action}} the sidebar", { action: expandSidebar ? t("Collapse") : t("Expand") })} />
        </ButtonGroup>
      </div>
      <Navbar className="AppSidebarFooterNavbar">
        <NavbarGroup align={Alignment.LEFT} className="AppSidebarFooterVersions">
          <NavbarHeading>
            ...
            <strong>CLI</strong> <span className="AppSidebarProgramString" title={programTitle}>{programString}</span>
          </NavbarHeading>
        </NavbarGroup>
