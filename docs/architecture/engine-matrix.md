...
container-desktop supports **two engines × five host types = ten connection
hosts**. Each is one entry in the registry, mapping a `(engine, host)` pair to the
three composition units (see [backend.md](backend.md)). This page is the lookup
...
- **Scope type** (`ControllerScopeType`) names the kind of thing the controller
  ...
  connection.
- **API base URL** seed differs by engine: Podman → `http://d`, Docker →
  `http://localhost` (the host part is irrelevant — requests go over the socket,
  not TCP).
...
`(engine, host)` pair throws — the ten entries are the whole supported surface.
