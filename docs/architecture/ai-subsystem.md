...
flowchart TB
  ...
  subgraph main[Electron main process — Node]
    ...
    runner["agent runner + tools<br/>AI-SDK tool loop · step cap"]:::component
...
**record**. Gated tools are `runCommand` and `webSearch`; the built-in knowledge lookup has no host or
network effect and runs freely in every mode.
...
```mermaid
flowchart TD
  call["gated tool call<br/>{ program, args } or web query"] --> mode{permission mode}
...
a `tool` timeline (commands, results, approval requests, rejections) over the multiplexed
`ai:stream-event` push channel.
...
of replaying, mirroring a real model so mock mode never loops on the approval card. Wiring is a single
...
| Path | Role |
| ---- | ---- |
| `core/permissions.ts` | `AIPermissionMode`, `resolveToolAction`, `commandKey`, `cachedVerdict`, the cache + store-port types |
| `core/channels.ts` | IPC channel names + payload contracts (`ai:chat`, `ai:agent:resolve`, `ai:permissions:*`, …) + `IAI`/`IAIBus` |
| `core/{ports,types,settings,egress,redact,providers,chatStore}.ts` | ports, settings normalization, egress classifier, redaction, provider catalog, chat-session port |
| `host/broker.ts` | `AIBroker` — gates, unified chat handler, resolve/persist/resume, permissions channels, per-stream state |
| `runtimes/node/permissionsStore.ts` | file-backed allow/reject record (fail-closed) |
| `runtimes/node/agent/{sandbox,sandboxExec,tools,agent,webSearch}.ts` | floor + executor, args-array spawn, gated tools, AI-SDK runner, SSRF-guarded web search |
...
| `ui/core/stores/aiStore.ts`, `ui/core/transcript.ts` | the one store + the ordered-timeline reducer |
| `ui/react/stores/useAIStore.ts` | React hook + the diagnostics-bundle collector |
| `adapters/electron/main/createElectronAISystem.ts` | composition root (assembles broker + runtimes / mocks) |
...
- **DI for Electron-coupled pieces** — `permissionsStore` (temp fs: ok/missing/error fail-closed, exclusive
  ...
  *enforcement* without Electron: mode wiring, fail-closed forces ask, `resolve` runs once / never on
  reject / sender-guarded, remember persists, the permissions channels.
- **Agent stack, TDD** — sandbox floor + `executeSandboxed` (scrubbed env, `enforceFloor:false` bypass,
  ...
  schema + the gating), the AI-SDK runner, the converged store.
